...
import { isAboutAI, isNewsStory, tagTopics } from './topics.mjs';
...
export const LIMITS = {
  maxAgeDays: 30,
  perSource: 40, // new stories taken from one build of a source (all its feeds together)
  keepPerSource: 80, // stories remembered per source across builds
  keepWebSearch: 250, // the same, for the per-AI Google News searches
  totalItems: 2500,
};
...
export function normalizeEntries(source, entries, now = Date.now(), { perSource = LIMITS.perSource } = {}) {
  ...
  return entries
    ...
    .filter((e) => isNewsStory(e.title) && (!source.aiOnly || isAboutAI(e.title)))
    ...
    .map((e) => {
      ...
      return {
        ...
        ...(e.fromSection ? { fromSection: true } : {}),
...
/**
 * Feeds only hold their latest 10–100 entries, so a story drops out of a feed
 * within hours or days. To keep the promise of "the last 30 days", each build
 * combines what it just fetched for a source with what the previous snapshot
 * had for it: fresh copies win, stories older than the age limit go, and the
 * newest `limit` are kept. Kept stories are re-tagged (and re-filtered for
 * `aiOnly` sources) so rule changes apply to them too.
 */
export function carryOver(source, fresh, previousItems = [], now = Date.now(), limit = LIMITS.keepPerSource, { recheckSections = false } = {}) {
  const cutoff = now - LIMITS.maxAgeDays * DAY;
  // Filtered stories are checked again, so a stricter AI filter also clears out what was kept before.
  // `recheckSections` covers stories kept before section-feed stories were marked (`fromSection`).
  const filtered = (i) => source.aiOnly || i.fromSection || (recheckSections && source.more?.length);
  const kept = previousItems
    .filter((i) => i.source === source.id && Date.parse(i.published) >= cutoff)
    .filter((i) => isNewsStory(i.title) && (!filtered(i) || isAboutAI(i.title)))
    .map((i) => ({ ...i, topics: tagTopics(`${i.title} ${i.summary}`) }));

  return mergeItems([fresh, kept]).slice(0, limit);
}
...
export function reusePrevious(items, previousItems = []) {
  ...
  return items.map((item) => {
    ...
    // The story keeps its illustration (its own, or the pool picture it was given).
    if (!item.cover && prev.cover?.startsWith('covers/')) next.cover = prev.cover;
    // Pictures: an earlier build may have looked one up, swapped the feed's small
    // image for a sharper one (keeping the original in `feedImage`), measured
    // it, or rejected it as too small. Reuse that verdict while the feed still
    // offers the same image.
    const sameFeedImage = item.image !== null && item.image === (prev.feedImage ?? prev.image);
    if ((item.image === null || sameFeedImage) && typeof prev.image === 'string') {
      next.image = prev.image;
      if (prev.feedImage) next.feedImage = prev.feedImage;
      if (prev.imageWidth) next.imageWidth = prev.imageWidth;
      if (prev.keepFeedImage) next.keepFeedImage = true;
