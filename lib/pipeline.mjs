...
import { tagTopics } from './topics.mjs';
...
export function normalizeEntries(source, entries, now = Date.now()) {
  const cutoff = now - LIMITS.maxAgeDays * DAY;

  return entries
    .filter((e) => e.published.getTime() >= cutoff)
    .sort((a, b) => b.published - a.published)
    .slice(0, LIMITS.perSource)
    .map((e) => {
      let { summary } = e;
      let discussion = null;
...
export function mergeItems(lists) {
  ...
  for (const list of lists) {
    for (const item of list) {
      ...
      if (seenUrls.has(u) || (t.length > 12 && seenTitles.has(t))) continue;
      seenUrls.add(u);
      seenTitles.add(t);
      merged.push(item);
    }
 ...
 * and teaser) so we don't re-scrape every article every hour.
