...
const STYLE = 'Editorial illustration for a technology news website. Dark navy and near-black background, glowing neon violet, cyan and magenta accents, soft cinematic volumetric light, clean modern composition with depth, high detail. No text, no letters, no numbers, no logos, no watermarks, no human faces.';
...
/** A stable pick from a theme's pool for a story that has no cover of its own. */
export function poolCover(pool, item) {
