 ...
 *   { title, url, summary, image, published: Date, publisher }
 * `publisher` is the RSS <source> element when present (aggregators like
 * Google News use it to name the original outlet), otherwise null.
...
export function parseFeed(xml, feedUrl) {
  ...
  for (const [, block] of xml.matchAll(blockRe)) {
    ...
    entries.push({
      ...
      published,
      publisher: firstText(block, ['source']) || null,
    });
  }
  return entries;
}
